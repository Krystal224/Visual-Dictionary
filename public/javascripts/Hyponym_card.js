...
Card.prototype.CreateVisualElement = function(){
    // div
    let node = document.createElement('div');
    let index = this.synset.Synset_name;
...
var  Create_Hypon_card = function(data){
    // console.log(data);
    let synsets = [];
    for(let sd of data){
        let synset = new Card(sd);
        synsets.push(synset);
    }
    // console.log(synsets);
    let synsetVisuals = d3.select('#hyponym_card').selectAll('.synset').data(synsets)
        .enter()
        .insert(function(d){
